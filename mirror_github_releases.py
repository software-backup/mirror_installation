...
TARGET_REPO_NAME     = os.environ.get('TARGET_REPO_NAME', 'unknown')  # 新增

# 状态文件目录与路径
STATUS_DIR         = os.path.join('status', TARGET_REPO_NAME)
SYNCED_DATA_FILE   = os.path.join(STATUS_DIR, 'synced_data.json')
SYNCED_DATA_BACKUP = f"{SYNCED_DATA_FILE}.bak"

# 确保目录存在
os.makedirs(STATUS_DIR, exist_ok=True)

SOURCE_OWNER, SOURCE_REPO_NAME_SRC = SOURCE_REPO.split('/')
...
def retry_upload(target_release, file_path, name, content_type):
    for attempt in range(1, RETRY_COUNT + 1):
        try:
            ...
            print(f"尝试上传 {name}（{attempt}/{RETRY_COUNT}）")
            uploaded = target_release.upload_asset(file_path, name=name, content_type=content_type)
            if uploaded:
                return uploaded
            print("上传返回 None，重试中...")
        except GithubException as e:
            if e.status == 422:
                print("检测到文件冲突，强制删除后重试...")
                delete_existing_asset(target_release, name)
            else:
                print(f"上传失败: {e}，{RETRY_DELAY}s 后重试")
        except Exception as e:
            print(f"上传失败: {e}，{RETRY_DELAY}s 后重试")
...
def sync_source_code(tag_name, target_release, synced_data):
    ...
    source_files = {
        f"SourceCode_{tag_name}.zip":
            f"https://github.com/{SOURCE_OWNER}/{SOURCE_REPO_NAME_SRC}/archive/refs/tags/{tag_name}.zip",
        f"SourceCode_{tag_name}.tar.gz":
            f"https://github.com/{SOURCE_OWNER}/{SOURCE_REPO_NAME_SRC}/archive/refs/tags/{tag_name}.tar.gz"
    }
    ...
    for filename, url in source_files.items():
        ...
        try:
            ...
            uploaded = retry_upload(target_release, temp_path, filename, "application/zip")
            if uploaded:
                synced_data['source_codes'][tag_name][filename] = {
                    'exists': True,
                    'synced_at': str(datetime.datetime.now())
    ...
    for asset in source_assets:
        ...
        if asset_key not in synced_data['assets'][source_id]:
            ...
            if source_info['size'] != target_info['size']:
                ...
                src_ts = datetime.datetime.fromisoformat(source_info['updated_at']).timestamp()
                tgt_ts = datetime.datetime.fromisoformat(target_info['updated_at']).timestamp()
                if src_ts > tgt_ts:
                    need_sync = True
        ...
        try:
            ...
            uploaded = retry_upload(target_release, temp_path, asset_name, content_type)
            if uploaded:
                actual_info = get_asset_info(uploaded)
                ...
                save_synced_data(synced_data)
                print(f"同步成功 {asset_name}")
                has_changes = True
            else:
                print(f"同步 {asset_name} 失败")
        except Exception as e:
            print(f"处理 {asset_name} 失败: {e}")
        finally:
            if os.path.exists(temp_path):
                os.remove(temp_path)
...
def get_or_create_release(target_repo, tag_name, name, body, draft, prerelease):
    ...
    try:
        try:
            ...
            commit_sha = target_repo.get_branch(default_branch).commit.sha
            target_repo.create_git_ref(ref=f"refs/tags/{tag_name}", sha=commit_sha)

        release = target_repo.create_git_release(
            tag=tag_name, name=release_name, message=body or "",
...
def main():
    ...
    try:
        ...
        for release in source_releases:
            ...
            has_any_changes = code_changes or asset_changes
